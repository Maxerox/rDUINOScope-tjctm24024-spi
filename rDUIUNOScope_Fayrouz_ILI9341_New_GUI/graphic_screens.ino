...
void removeTime_addXX() {
  if (dateEntryPos == 0) {
    tft.fillRect(38, 41, 130, 13, l_text);
    tft.fillRect(143, 41, 80, 13, l_text );
    tft.setTextColor(BLACK, l_text);
    tft.cursorToXY(41, 45);
    tft.print("XX/XX/XXXX");
    tft.cursorToXY(146, 45);
    tft.print("XX:XX");
  }
}
///////////////////////////////////////////////////// GPS Screen Graphics ///////////////////////////////////////////////////////
void drawGPSScreen()
{
  ...
  tft.fillRect(0, 0, 240, 20, title_bg); // yellow Rectangle  Was (1, 1, 239, 35, title_bg);
  tft.setTextScale(2);
  ...
  tft.fillRect(0, 20, 240, 300, Black_area);
  if (IS_NIGHTMODE) {
    drawBin("UI/night/bg_gps.bin", 0, 20, 240, 300);
  } else {
    drawBin("UI/day/bg_gps.bin", 0, 20, 240, 300);
  }

  // CONTINUE BTN
  tft.setTextScale(2);
  tft.setTextColor(title_texts);
  tft.cursorToXY(80, 283);
...
{
  CURRENT_SCREEN = 1;
  tft.fillRect(0, 0, 240, 20, title_bg);
  tft.cursorToXY(47, 3);
  tft.setTextColor(title_texts);
  tft.setTextScale(2);
  tft.print("Date & Time");
  ...
  if (SD.open(OBJ_NAME)) {
    ...
    drawBin(obj_name, 0, 20, 240, 300); // Drawing selected Object Tumbnail 140*140 Pixels
  }
  ...
  tft.drawRect(11, 149, 60, 33, messie_btn); //1 x 15>95   y 149>274
  tft.drawRect(90, 149, 60, 33, messie_btn);//2 x 120>200 y 149>274
  tft.drawRect(172, 149, 60, 33, messie_btn);//3 x 230>280 y 149>274
  tft.drawRect(11, 190, 60, 33, messie_btn); //4 x 15>95   y 190> 335
  tft.drawRect(90, 190, 60, 33, messie_btn);//5 x 120>200 y 190>335
  tft.drawRect(172, 190, 60, 33, messie_btn);//6 x 230>280 y 190>335
  tft.drawRect(11, 231, 60, 33, messie_btn); //7 x 15>95   y 231>396
  tft.drawRect(90, 231, 60, 33, messie_btn);//8 x 120>200 y 231>396
  tft.drawRect(172, 231, 60, 33, messie_btn);//9 x 230>280 y 231>396
  tft.drawRect(90, 272, 60, 33, messie_btn);//0 x 120>200 y 408>478
...
void drawMainScreen()
{
  ...
  {
    OBJ_NAME_1.toCharArray(obj_name, 50);
    drawBin(obj_name, 0, 59, 240, 18); // Drawing selected Object Tumbnail 140*140 Pixels
    tft.drawLine(0, 60, 240, 60, BLACK );//Time_area_back
  }

  drawStatusBar();
  tft.fillRect(0, 20, 240, 42, Time_area_back );//ILI9341_INDIGO // title_bg / ILI9341_SLATEBLUE /ILI9341_DARKBLUE/ ILI9341_DODGERBLUE
  tft.setTextColor(Time_area_font); // title_texts // ILI9341_INDIGO ILI9341_DARKBLUE
  tft.cursorToXY(1, 23);
  tft.setTextScale(2);
  tft.print("TIME:");
  tft.cursorToXY(66, 23);
  tft.print(String(rtc.getTimeStr()).substring(0, 5));

  tft.cursorToXY(5, 43);
  tft.print("LST");
  tft.cursorToXY(55, 43);
  tft.print(":");
  tft.cursorToXY(66, 43);
  if ((int)LST < 10) {
    tft.print("0");
    tft.setTextScale(1);
    tft.print(" ");
  ...
  tft.cursorToXY(150, 25);
  tft.print("TEMP:");
  tft.cursorToXY(154, 47);
  tft.print("HUM");
  tft.cursorToXY(186, 47);
  tft.print(":");
  tft.cursorToXY(196, 25);
  tft.print(_temp, 0);
  tft.cursorToXY(220, 20);
  tft.setTextScale(2);
  tft.print("o");
  tft.setTextScale(2);
  tft.cursorToXY(221, 25);
  tft.print(" C");
  tft.cursorToXY(196, 47);
  tft.print(_humid, 0);
  tft.cursorToXY(217, 47);
  tft.print(" %");
  ...
  tft.cursorToXY(82, 66);
  tft.print("@");
  tft.setTextColor(l_text);
  tft.cursorToXY(90, 66);
  tft.print("LAT:");
  tft.setTextColor(btn_l_border);
  ...
  if (OBJECT_NAME != "") {
    ...
    if ((OBJECT_NAME != "") && (ALT < 0)) {
      tft.setTextScale(1);
      tft.cursorToXY(0, 193);
      tft.setTextColor(RED);
      tft.println("OBJECT NOT VISIBLE!");
      tft.fillRect(187, 157, 52, 47, Maroon);
      tft.setTextScale(3);
      tft.cursorToXY(204, 168);
      tft.print("!");
    }
    else
    {
      ...
      {
        OBJ_NAME.toCharArray(obj_name, 50);
        drawBin(obj_name, 0, 95, 240, 95); // Drawing selected Object Tumbnail 320*133 Pixels // Make it 145
      }
      if (SD.open(OBJ_NAME_2) /*&& (IS_TRACKING)*/)
      {
        OBJ_NAME_2.toCharArray(obj_name, 50);
        drawBin(obj_name, 0, 191, 240, 95); // Drawing Object Details Background Image
        tft.cursorToXY(0, 197);
        tft.setTextColor(l_text);
        tft.setTextScale(1);
        tft.println(OBJECT_DETAILS);
        tft.drawLine(0, 190, 240, 190, title_bg);
      } else {
        tft.drawLine(0, 190, 240, 190, title_bg);
        OnScreenMsg(1);
      }
    ...
    if ((OBJECT_NAME != "CP") && (IS_BT_MODE_ON == false)) {
      tft.setTextColor(l_text);
      tft.cursorToXY(0, 143);
      tft.println("HA :");
      tft.cursorToXY(36, 143);
      tft.setTextColor(title_bg);
      tft.print(HAHour, 0);
      tft.print("h ");
      tft.print(HAMin, 1);
      tft.print("m");
      tft.cursorToXY(0, 160);
      tft.setTextColor(l_text);
      tft.println("ALT:");
      tft.cursorToXY(0, 177);
      tft.println("AZ :");
    }
    ...
    tft.cursorToXY(0, 100);
    tft.println("");
    tft.println("No object is selected!");
    tft.println("");
    tft.setTextScale(1);
    tft.println("Use LOAD button below to select objects \nfrom Solar System, Messier, NGC or \ncustom.csv catalogues");
  }
...
void drawCoordinatesScreen() {
  ...
  if (SD.open(OBJ_NAME_3)/* && (IS_TRACKING)*/)
  {
    ...
    drawBin(obj_name, 184, 0, 56, 20); // Drawing selected Object Tumbnail 140*140 Pixels
  }
...
void drawLoadScreen() {
  ...
  tft.fillScreen(BLACK);
  tft.fillRect(0, 0, 300, 20, title_bg); // yellow Rectangle  Was (1, 1, 239, 35, title_bg);
  tft.setTextColor(title_texts);
  tft.setTextScale(2);
  tft.cursorToXY(5, 3);
  tft.print("Load...");
  ...
  if (SD.open(OBJ_NAME_4))
  {
    ...
    drawBin(obj_name, 0, 20, 240, 300); // Draw Back Button Thumbnail
  }
  ...
  {
    OBJ_NAME_3.toCharArray(obj_name, 50);
    drawBin(obj_name, 184, 0, 56, 20); // Draw Back Button Thumbnail
  }
  ...
  {
    OBJ_NAME_2.toCharArray(obj_name, 50);
    drawBin(obj_name, 0, 289, 240, 31); // Draw Next/Prev Button Thumbnail
  }
  drawLoadObjects();
}
...
void drawOptionsScreen()
{
  ...
  tft.fillRect(0, 0, 240, 20, title_bg); // yellow Rectangle  Was (1, 1, 239, 35, title_bg);
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/btn_back.bin", 184, 0, 56, 20); // Draw Back Button Thumbnail
  } else {
    drawBin("UI/night/btn_back.bin", 184, 0, 56, 20); // Draw Back Button Thumbnail
  }
  tft.cursorToXY(5, 3);
  tft.setTextColor(title_texts);
  tft.setTextScale(2);
  tft.print("OPTIONS");
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/bg_options_buttons_off.bin", 0, 20, 240, 300); // All Page Buttons off - Day
  } else {
    drawBin("UI/night/bg_options_buttons_off.bin", 0, 20, 240, 300); // All Page Buttons off - Night
  }
...
void drawSTATScreen() {
  ...
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/btn_back.bin", 184, 0, 56, 20); // Draw Back Button Thumbnail
    drawBin("UI/day/stats_background.bin", 0, 20, 240, 300); // Draw Back Button Thumbnail

  } else {
    drawBin("UI/night/btn_back.bin", 184, 0, 56, 20); // Draw Back Button Thumbnail
    drawBin("UI/night/stats_background.bin", 0, 20, 240, 300); // Draw Back Button Thumbnail
  }
  tft.drawLine(0, 22, 240, 22, title_bg); // yellow line
  tft.cursorToXY(5, 3);
  tft.setTextColor(title_texts);
  tft.setTextScale(2);
  tft.print("Stats");
...
void drawStarSyncScreen() {
  ...
  tft.fillRect(0, 0, 240, 20, title_bg); // yellow Rectangle  Was (1, 1, 239, 35, title_bg);
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/btn_done.bin", 183, 0, 56, 20); // Draw Back Button Thumbnail
  } else {
    drawBin("UI/night/btn_done.bin", 183, 0, 56, 20); // Draw Back Button Thumbnail
  }
  tft.cursorToXY(5, 5);
  tft.setTextColor(title_texts);
  tft.setTextScale(2);
  ...
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/btn_next_prev.bin", 0, 289, 240, 31); // Draw Next/Prev Button Thumbnail
  } else {
    drawBin("UI/night/btn_next_prev.bin", 0, 289, 240, 31); // Draw Next/Prev Button Thumbnail
  }
...
void drawConstelationScreen(int indx)
{
  ...
  tft.fillRect(0, 20, 240, 300, Black_area);
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/manual_align_background.bin", 0, 20, 240, 300); // Draw Back Button Thumbnail
  } else {
    drawBin("UI/night/manual_align_background.bin", 0, 20, 240, 300); // Draw Back Button Thumbnail
  }
  tft.cursorToXY(12, 3);
  tft.setTextColor(title_texts);
  tft.setTextScale(2);
  ...
  if (ALLIGN_TYPE != 3)
  {
    ...
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_sync_repeat.bin", 0, 289, 240, 31); // Draw REPEAT/SYNC! Button Thumbnail
    } else {
      drawBin("UI/night/btn_sync_repeat.bin", 0, 289, 240, 31); // Draw REPEAT/SYNC! Button Thumbnail
    }
  }
  else
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_sync_exit.bin", 0, 289, 240, 31); // Draw <EXIT/SYNC! Button Thumbnail
    } else {
      drawBin("UI/night/btn_sync_exit.bin", 0, 289, 240, 31); // Draw <EXIT/SYNC! Button Thumbnail
    }
  }
  if (ALLIGN_STEP == 1)
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_sync_exit.bin", 0, 289, 240, 31); // Draw REPEAT/SYNC! Button Thumbnail
    } else {
      drawBin("UI/night/btn_sync_exit.bin", 0, 289, 240, 31); // Draw REPEAT/SYNC! Button Thumbnail
    }
  ...
  else if (ALLIGN_STEP == 2)
  {
    ...
    {
      if (!IS_NIGHTMODE) {
        drawBin("UI/day/btn_next_exit.bin", 0, 289, 240, 31); // Draw <EXIT/NEXT> Button Thumbnail
      } else {
        drawBin("UI/night/btn_next_exit.bin", 0, 289, 240, 31); // Draw <EXIT/NEXT> Button Thumbnail
      }
      tft.cursorToXY(0, 53);
      ...
      tft.cursorToXY(1, 183);
      tft.print("Now Observing:");
      tft.setTextColor(l_text);
      tft.cursorToXY(1, 200);
      tft.println(OBJECT_NAME);
    }
    else
    {
      DrawButton(165, 280, 75, 40, "CENTER", 0, btn_l_border, btn_l_text, 2, false);
      tft.cursorToXY(0, 53);
...
void OnScreenMsg(int Msg) {
  ...
  if (SD.open(OBJ_NAME)/* && (IS_TRACKING)*/) {
    OBJ_NAME.toCharArray(obj_name, 50);
    drawBin(obj_name, 49.875, 127, 140, 67); // Drawing selected Object Tumbnail 140*140 Pixels
  }
  ...
  if (Msg == 2) {
    ...
    m2 = "TRACKING was";
    m3 = "Turned Off!";
    tft.cursorToXY(73, 153);
    tft.setTextScale(3);
    tft.println(m1);
    tft.cursorToXY(66, 177);
    tft.setTextScale(2);
    tft.print(m2);
    tft.cursorToXY(73, 193);
    tft.print(m3);
    delay(1000);
    drawMainScreen();
  } else if (Msg == 4) {
    ...
    //Timer3.stop(); //
    //IS_TRACKING = false;
    tft.cursorToXY(71, 153);
    tft.setTextScale(3);
    tft.println(m1);
    tft.cursorToXY(71, 177);
    tft.setTextScale(2);
    tft.print(m2);
    tft.cursorToXY(71, 193);
    tft.print(m3);
  } else if (Msg == 8) {
    m1 = "Connected";
    m2 = "TO";
    m3 = "PHD2:)";
    tft.cursorToXY(79, 153);
    tft.setTextScale(2);
    tft.println(m1);
    tft.cursorToXY(109, 173);
    tft.setTextScale(2);
    tft.print(m2);
    tft.cursorToXY(94, 193);
    tft.print(m3);
    delay(1000);
  } else if (Msg == 9) {
    m1 = "Disconnected";
    m2 = "FROM";
    m3 = "PHD2:)";
    tft.cursorToXY(65, 153);
    tft.setTextScale(2);
    tft.println(m1);
    tft.cursorToXY(101, 173);
    tft.setTextScale(2);
    tft.print(m2);
    tft.cursorToXY(94, 193);
    tft.print(m3);
...
void drawMainScreen_Menu(int fill) {
  if (MAIN_SCREEN_MENU == 0) {
    if (IS_BT_MODE_ON == false) {
      if (!IS_NIGHTMODE) {
        drawBin("UI/day/btn_load.bin", 7, 250, 68, 29);
        drawBin("UI/day/btn_home.bin", 86, 250, 68, 29);
        drawBin("UI/day/btn_stats.bin", 7, 285, 68, 29);
        drawBin("UI/day/btn_up_down.bin", 86, 285, 68, 29);
        drawBin("UI/day/btn_BT_off.bin", 166.5, 285, 68, 29);
      } else {
        drawBin("UI/night/btn_load.bin", 7, 250, 68, 29);
        drawBin("UI/night/btn_home.bin", 86, 250, 68, 29);
        drawBin("UI/night/btn_stats.bin", 7, 285, 68, 29);
        drawBin("UI/night/btn_up_down.bin", 86, 285, 68, 29);
        drawBin("UI/night/btn_BT_off.bin", 166.5, 285, 68, 29);
      }
    } else {
      if (!IS_NIGHTMODE) {
        drawBin("UI/day/btn_up_down.bin", 86, 285, 68, 29);
        drawBin("UI/day/btn_BT_on.bin", 166.5, 285, 68, 29);
      } else {
        drawBin("UI/night/btn_up_down.bin", 86, 285, 68, 29);
        drawBin("UI/night/btn_BT_on.bin", 166.5, 285, 68, 29);
      }
    }
    ...
    if (IS_TRACKING == true) {
      ...
      tft.setTextScale(2);
      tft.fillRect(0, 77, 240, 17, BLACK);
      tft.cursorToXY(1, 79);
      tft.setTextColor(btn_l_border);
      if (OBJECT_NAME.length() > 7) {
        tft.setTextScale(1);
    ...
    if (IS_BT_MODE_ON == false) {
      if (!IS_NIGHTMODE) {
        drawBin("UI/day/btn_coord.bin", 7, 250, 68, 29);
        drawBin("UI/day/btn_map.bin", 86, 250, 68, 29);
        drawBin("UI/day/btn_option.bin", 7, 285, 68, 29);
        drawBin("UI/day/btn_up_down.bin", 86, 285, 68, 29);
      } else {
        drawBin("UI/night/btn_coord.bin", 7, 250, 68, 29);
        drawBin("UI/night/btn_map.bin", 86, 250, 68, 29);
        drawBin("UI/night/btn_option.bin", 7, 285, 68, 29);
        drawBin("UI/night/btn_up_down.bin", 86, 285, 68, 29);
      }
    } else {
      if (!IS_NIGHTMODE) {
...
void TimerUpdateDraw(int z) {
  ...
  if (dateEntryPos >= 0 && dateEntryPos < 2) {
    tft.fillRect((dateEntryPos * 11) + 37, 45, 11, 13, l_text); // 15 was 18
    tft.cursorToXY((dateEntryPos * 11) + 37, 45);
  } else if (dateEntryPos > 1 && dateEntryPos < 4) {
    tft.fillRect((dateEntryPos * 11) + 37, 45, 11, 13, l_text);
    tft.cursorToXY((dateEntryPos * 11) + 37, 45);
  } else if (dateEntryPos > 3 && dateEntryPos < 8) {
    tft.fillRect((dateEntryPos * 11) + 37, 45, 11, 13, l_text);
    tft.cursorToXY((dateEntryPos * 11) + 37, 45);
  } else if (dateEntryPos > 7 && dateEntryPos < 10) {
    tft.fillRect(((dateEntryPos - 8) * 11) + 146, 45, 11, 13, l_text);
    tft.cursorToXY(((dateEntryPos - 8) * 11) + 146, 45);
...
void drawStarMap() {
  ...
  if (!IS_CUSTOM_MAP_SELECTED) {
    ...
    if ((DEC_dd < 70) && (DEC_dd > -70)) {
      telescope_X = 240 - (79.5 * ((curr_RA_H + curr_RA_M / 45) - (tmp_map_c - 1) * 2.25));

      if (tmp_map_r == 2) {

        if ((DEC_dd > -20) && (DEC_dd < 20)) {
          telescope_Y = 170 + DEC_dd * -4.89;  //  Was   -5.35  Done
          // Example Targets M 15,M53    //M5, M77, M61 > 0 //M2, M14, M73 <0
        } else if (DEC_dd > 20) {
          telescope_Y = 86.65 - (DEC_dd - 20) * 6.06; //     Done
          // Example Targets  M1,M64,M3
        } else if (DEC_dd < -20) {
          telescope_Y = 460 + (abs(DEC_dd) - 20) * 6.06; //   Done
          // Example Targets  M4, M30, M75,M83
        }
      } else {
        if ((abs(DEC_dd) > 30) && (abs(DEC_dd) < 40)) {
          telescope_Y = 320 - (DEC_dd - 30) * 4.79;        // Done
          // Example Targets  M13,M36, M37, M38,M57
        } else if ((abs(DEC_dd) > 40) && (abs(DEC_dd) < 60)) {
          telescope_Y = 275 - (DEC_dd - 40) * 6.93;              // 438         // Done
          // Example Targets   M110 , M92, M39, M102,M101, M51,M63, M76
        } else if (DEC_dd > 60) {
          telescope_Y = 161.98 - (DEC_dd - 60) * 12.46;   // Done
          // Example Targets  M82, M81
        }
        if (DEC_dd < 0) {
          telescope_Y = 300 - telescope_Y; // Was 480
        }
      }
    }
  }
  ...
  drawBin(My_Map, 0, 20, 240, 300);

  if (!IS_CUSTOM_MAP_SELECTED) {
    tft.drawCircle(telescope_X, telescope_Y, 10, btn_l_border);
    tft.drawLine(0, telescope_Y, 240, telescope_Y, btn_l_border);
    tft.drawLine(telescope_X, 30, telescope_X, 320, btn_l_border);
  }
  IS_CUSTOM_MAP_SELECTED = false;
}
...
{
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/status_bar_back.bin", 0, 0, 240, 18); // Drawing selected Object Tumbnail 140*140 Pixels
  } else {
    drawBin("UI/night/status_bar_back.bin", 0, 0, 240, 18); // Drawing selected Object Tumbnail 140*140 Pixels
  }

  tft.drawLine(0, 17, 240, 17, BLACK );//
  tft.setTextScale(1);
  ...
  tft.cursorToXY(109, 2);
  tft.print("Mer-F");

  tft.cursorToXY(141, 2);
  tft.print("F1");

  tft.cursorToXY(151, 2);
  tft.print("F2");

  tft.cursorToXY(179, 2);
  tft.print("Sound");

  tft.cursorToXY(210, 2);
  tft.print("Motors");
  ...
  tft.cursorToXY(22, 12);
  tft.print("%");

  if (TFT_Time == "AL-ON") {
    tft.cursorToXY(35, 12);
    tft.setTextColor(Button_State_ON);
    tft.print("AL-ON");
  }

  if (TFT_Time == "30 s") {
    tft.cursorToXY(37, 12);
    tft.setTextColor(Button_State_OFF);
    tft.print("30 S");
  }
...
void updateTrackingMode_opt()
{
  if (Tracking_type == 0) {   // 1: Sidereal, 2: Solar, 0: Lunar;
    ...
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_lunar_on.bin", 158, 279, 73, 33); // Lunar Button on
      drawBin("UI/day/btn_celestial_off.bin", 158, 214, 73, 33); //Celestial Button off
      drawBin("UI/day/btn_solar_off.bin", 158, 247, 73, 33); // Solar Button off
    } else {
      drawBin("UI/night/btn_lunar_on.bin", 158, 279, 73, 33); // Lunar Button on
      drawBin("UI/night/btn_celestial_off.bin", 158, 215, 73, 33); //Celestial Button off
      drawBin("UI/night/btn_solar_off.bin", 158, 247, 73, 33); // Solar Button off
    }
  } else if (Tracking_type == 1) {  // 1: Sidereal, 2: Solar, 0: Lunar;
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_celestial_on.bin", 158, 214, 73, 33); //Celestial Button on
      drawBin("UI/day/btn_solar_off.bin", 158, 247, 73, 33); // Solar Button off
      drawBin("UI/day/btn_lunar_off.bin", 158, 279, 73, 33); // Lunar Button off
    } else {
      drawBin("UI/night/btn_celestial_on.bin", 158, 215, 73, 33); //Celestial Button on
      drawBin("UI/night/btn_solar_off.bin", 158, 247, 73, 33); // Solar Button off
      drawBin("UI/night/btn_lunar_off.bin", 158, 279, 73, 33); // Lunar Button off
    }
  } else if (Tracking_type == 2) {  //  1: Sidereal, 2: Solar, 0: Lunar;
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_solar_on.bin", 158, 247, 73, 33); // Solar Button on
      drawBin("UI/day/btn_celestial_off.bin", 158, 214, 73, 33); //Celestial Button off
      drawBin("UI/day/btn_lunar_off.bin", 158, 279, 73, 33); // Lunar Button off
    } else {
      drawBin("UI/night/btn_solar_on.bin", 158, 247, 73, 33); // Solar Button on
      drawBin("UI/night/btn_celestial_off.bin", 158, 215, 73, 33); //Celestial Button off
      drawBin("UI/night/btn_lunar_off.bin", 158, 279, 73, 33); // Lunar Button off
    }
  }
...
void updateScreenTimeout_opt()
{
  if (TFT_timeout == 0) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_never_on.bin", 3, 147, 40, 32); // NEVER Button on
    } else {
      drawBin("UI/night/btn_never_on.bin", 3, 147, 40, 32); // NEVER Button on
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_never_off.bin", 3, 147, 40, 32); // NEVER Button off
    } else {
      drawBin("UI/night/btn_never_off.bin", 3, 147, 40, 32); // NEVER Button off
    }
  }
  if (TFT_timeout == 30000) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_30s_on.bin", 42, 147, 40, 32); // 30S Button on
    } else {
      drawBin("UI/night/btn_30s_on.bin", 42, 147, 40, 32); // 30S Button on
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_30s_off.bin", 42, 147, 40, 32); // 30S Button off
    } else {
      drawBin("UI/night/btn_30s_off.bin", 42, 147, 40, 32); // 30S Button off
    }
  }
  if (TFT_timeout == 60000) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_60s_on.bin", 81, 147, 40, 32); // 60S Button on
    } else {
      drawBin("UI/night/btn_60s_on.bin", 81, 147, 40, 32); // 60S Button on
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_60s_off.bin", 81, 147, 40, 32); // 60S Button off
    } else {
      drawBin("UI/night/btn_60s_off.bin", 81, 147, 40, 32); // 60S Button off
    }
  }
  if (TFT_timeout == 120000) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_2m_on.bin", 120, 147, 40, 32); // 2M Button on
    } else {
      drawBin("UI/night/btn_2m_on.bin", 120, 147, 40, 32); // 2M Button on
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_2m_off.bin", 120, 147, 40, 32); // 2M Button off
    } else {
      drawBin("UI/night/btn_2m_off.bin", 120, 147, 40, 32); // 2M Button off
    }
  }
  if (TFT_timeout == 300000) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_5m_on.bin", 159, 147, 40, 32); // 5M Button on
    } else {
      drawBin("UI/night/btn_5m_on.bin", 159, 147, 40, 32); // 5M Button on
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_5m_off.bin", 159, 147, 40, 32); // 5M Button off
    } else {
      drawBin("UI/night/btn_5m_off.bin", 159, 147, 40, 32); // 5M Button off
    }
  }
  if (TFT_timeout == 600000) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_10m_on.bin", 199, 147, 40, 32); // 10M Button on
    } else {
      drawBin("UI/night/btn_10m_on.bin", 198, 147, 40, 32); // 10M Button on
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_10m_off.bin", 199, 147, 40, 32); // 10M Button off
    } else {
      drawBin("UI/night/btn_10m_off.bin", 198, 147, 40, 32); // 10M Button off
    }
  }
}
...
void updateMeridianFlip_opt()
{
  if (IS_MERIDIAN_FLIP_AUTOMATIC) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_auto_on.bin", 83, 215, 70, 32); // Mer F Auto Button on
      drawBin("UI/day/btn_mer_off_off.bin", 83, 247, 70, 33); // Mer F off Button off
    } else {
      drawBin("UI/night/btn_auto_on.bin", 83, 215, 70, 32); // Mer F Auto Button on
      drawBin("UI/night/btn_mer_off_off.bin", 83, 247, 70, 32); // Mer F off Button off
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_auto_off.bin", 83, 215, 70, 32); // Mer F Auto Button off
      drawBin("UI/day/btn_mer_off_on.bin", 83, 247, 70, 33); // Mer F off Button on
    } else {
      drawBin("UI/night/btn_auto_off.bin", 83, 215, 70, 32); // Mer F Auto Button off
      drawBin("UI/night/btn_mer_off_on.bin", 83, 247, 70, 32); // Mer F off Button on
    }
  }
}
...
void updateSound_opt()
{
  if (IS_SOUND_ON) {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_sound_on.bin", 6, 108, 66, 17); // Sound Button on
    } else {
      drawBin("UI/night/btn_sound_on.bin", 6, 108, 66, 17); // Sound Button on
    }
  } else {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_sound_off.bin", 6, 108, 66, 17); // Sound Button off
    } else {
      drawBin("UI/night/btn_sound_off.bin", 6, 108, 66, 17); // Sound Button off
    }
  }
}
...
void updateStepper_opt()
{
  ...
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_steppers_on.bin", 86, 108, 66, 17); // Steppers Button on
    } else {
      drawBin("UI/night/btn_steppers_on.bin", 86, 108, 66, 17); // steppers Button on
    }
  }
  else
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_steppers_off.bin", 86, 108, 66, 17); // Steppers Button off
    } else {
      drawBin("UI/night/btn_steppers_off.bin", 86, 108, 66, 17); // steppers Button off
    }
  }
}
...
void updateFocus_stepper_opt()
{
  ...
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_focusM_on.bin", 168, 162, 66, 17); // Steppers Button on
    } else {
      drawBin("UI/night/btn_focusM_on.bin", 168, 162, 66, 17); // steppers Button on
    }
  }
  else
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_focusM_off.bin", 168, 162, 66, 17); // Steppers Button off
    } else {
      drawBin("UI/night/btn_focusM_off.bin", 168, 162, 66, 17); // steppers Button off
    }
  }
}
...
void updateTriangleBrightness_opt()
{
  ...
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_max_on.bin", 194, 36, 38, 30); // Max Button on
    } else {
      drawBin("UI/night/btn_max_on.bin", 194, 36, 38, 30); // Max Button on
    }
  }
  else
  {
    if (!IS_NIGHTMODE) {
      drawBin("UI/day/btn_max_off.bin", 194, 36, 38, 30); // Max Button off
    } else {
      drawBin("UI/night/btn_max_off.bin", 194, 36, 38, 30); // Max Button off
    }
  }

  //Dark Triangle
  tft.fillTriangle(4, 67, 190, 67, 190, 33, BLACK);

  //Clear Triangle
  int x_br = (TFT_Brightness - 144) * 4 + 1;
  int tria_h = floor((255 - x_br) * 0.2000);
  tft.fillTriangle(4, 67, x_br, 67, x_br, tria_h + 34, Bright_triangle);
...
{
  tft.fillRect(0, 53, 240, 227, BLACK);
  if (ALLIGN_TYPE == 3)
  {
    ...
    for (int i = 0; i < 6; i++) {
      for (int j = 0; j < 4; j++) {
        ...
        tft.fillRect(((j * 56) + 9), ((i * 33) + 57), 53, 30, messie_btn);
        int l = (S_NAME.length() / 2) * 6;
        tft.cursorToXY(((j * 56) + (33 - l)), ((i * 33) + 62));
        tft.setTextScale(1);
        tft.print(S_NAME);
        tft.setTextScale(2);
        tft.cursorToXY(((j * 56) + 22), ((i * 33) + 73));
        tft.print(C_NAME);
        kk += 1;
      }
    }
  ...
  if (LOAD_SELECTOR == 1)
  {
    ...
    for (int i = 0; i < 5; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        ...
        if (M_NAME == "")
        {
          tft.fillRect(((j * 56) + 9), ((i * 33) + 107), 53, 30, BLACK);
        }
        else
        {
          DrawButton(((j * 56) + 9), ((i * 33) + 107), 53, 30, M_NAME, messie_btn, 0, l_text, 2, false);
  ...
  else if (LOAD_SELECTOR == 2)
  {
    ...
    for (int i = 0; i < 5; i++) {
      for (int j = 0; j < 4; j++) {
        ...
        {
          DrawButton(((j * 56) + 9), ((i * 33) + 107), 53, 30, M_NAME, messie_btn, 0, l_text, 1, true);
          ll += 1;
        }
      }
    }
...
void drawAutoGuidingScreen() {
  CURRENT_SCREEN = 14;
  if (IS_NIGHTMODE) {
    drawBin("GUI/night/autoguiding.bin", 0, 0, 240, 321);
  } else {
    drawBin("GUI/day/autoguiding.bin", 0, 0, 240, 321);
  }
  DrawButton(7, 53, 225, 27, "RA+ Pulse", 0, btn_d_border, btn_d_border, 2, false);
  DrawButton(7, 90, 225, 27, "RA- Pulse", 0, btn_d_border, btn_d_border, 2, false);
  DrawButton(7, 127, 225, 27, "DEC+ Pulse", 0, btn_d_border, btn_d_border, 2, false);
  DrawButton(7, 163, 225, 27, "DEC- Pulse", 0, btn_d_border, btn_d_border, 2, false);
}
