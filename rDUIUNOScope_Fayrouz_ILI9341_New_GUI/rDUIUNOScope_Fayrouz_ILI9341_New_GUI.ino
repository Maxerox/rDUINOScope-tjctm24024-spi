...
const String FirmwareDate = "18/6/2020";
const String FirmwareNumber = "v0.0.2 Boiana-EQ-Conedera-ILI9341";
const String FirmwareName = "rDUINOScope Boiana-Conedera-EQ V0.0.2";
const String FirmwareTime = "12:00:00";
const String HardwareVersion = "Maxerox V.1.0";

// Default values to load when CANCEL button is hit on the GPS screen ( Change by your Home Location!)

float OBSERVATION_LONGITUDE = -58.6390;  // (24.7136* - Riyadh, Saudi Arabia) // (31.0004* - Tanta, Egypt)
float OBSERVATION_LATTITUDE = -34.6553; // (46.6753* - Riyadh, Saudi Arabia) // (30.7865* - Tanta, Egyp)
float OBSERVATION_ALTITUDE = 23.00;  // Riyadh, Saudi Arabia // 11.00 Tanta, Egypt
...
int WORMDEC = 65;
#endif
double REDUCTOR = 2.5;      // 1:4 gear reduction
...
#define SD_CS 2    // Chip Select for SD card
...
// These are the touchscreen pins 
#define TH_CS  22
#define TH_IRQ  24

int tzPrev = 0;

#define TS_MINX 380  // touch Left margin  >>> Default value is 110 
#define TS_MINY 320   // touch up margin    >>> Default value is 80
#define TS_MAXX 3780  // touch Right margin >>> Default value is 900  
#define TS_MAXY 3600  // touch Down margin  >>> Default value is 940 

#define MINPRESSURE 1000    // Minimum Pressure Touch Sensitivity >>> Default value is 10
#define MAXPRESSURE 3000 // Maximum Pressure Touch Sensitivity >>> Default value is 1000

XPT2046_Touchscreen ts(TH_CS, TH_IRQ);

///////////////////////////////////////////////////// Other Modules Setup /////////////////////////////////////////////////////

#define DHTPIN 43
#define DHTTYPE DHT11
DS3231 rtc(20, 21);           // (SDA-20, SCL-21) from the RTC board
DHT dht(DHTPIN, DHTTYPE);
TinyGPSPlus gps;             // (Rx-16, Tx-17)

/** NIGHT MODE ANALOG pin */
#define DAY_NIGHT_PIN A10
...
////////////////////////// Focus Motor Setup //////////////////////////////////
int Focus_EN = 38;
int focus_step_pin = 37;
int focus_dir_pin = 36;
...
int16_t Debug_color, Bright_triangle, Time_area_back, Black_area, Time_area_font, texts, Button_State_ON, Button_State_OFF, Button_Title, l_text, d_text, btn_l_border, btn_d_border, btn_l_text, btn_d_text, btn_l_selection, title_bg, title_texts, messie_btn, file_btn, ngc_btn, MsgBox_bg, MsgBox_t; // defines string constants for the clor - Depending on the DAY/NIGHT modes
File roots;
...
void setup(void) {
  ...
  if (!IS_NIGHTMODE) {
    drawBin("UI/day/hackaday_logo.bin", 0, 304, 90, 16);
  } else {
    drawBin("UI/night/hackaday_logo.bin", 0, 304, 90, 16);
  }
      ...
      if (p.z < MINPRESSURE || p.z > MAXPRESSURE) {
        return;
           ...
           //Useful to debug touch:
             ...
             Serial.println(tz);
             Serial.print(", z = ");
             Serial.println(tzPrev);
             // tft.drawPixel(tx, ty, Debug_color);
           #endif
      
      
      tzPrev = considerTouchInput(tx, ty, tz, tzPrev);
