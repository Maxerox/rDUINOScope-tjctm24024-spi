...
{ // UPDATEs time on Screen1 && Screen4 -  Clock Screen and Main Screen
  ...
  if (CURRENT_SCREEN == 4 && (millis() - update_time) > 10000)
  {
    ...
    {
      if (!IS_NIGHTMODE) {
        drawBin("UI/day/status_bar_new.bin", 0, 59, 240, 18); // Drawing selected Object Tumbnail 140*140 Pixels
      } else {
        drawBin("UI/night/status_bar_new.bin", 0, 59, 240, 18); // Drawing selected Object Tumbnail 140*140 Pixels
      }
      ...
      tft.cursorToXY(82, 67);
      tft.print("@");
      tft.setTextColor(l_text);
      tft.cursorToXY(90, 67);
      tft.print("LAT:");
      tft.setTextColor(btn_l_border);
    ...
    tft.fillRect(64, 43, 86, 15, Time_area_back);
  ...
  else if (CURRENT_SCREEN == 1 && (millis() - update_time) > 10000 && changes == 0)
  {
    ...
    tft.fillRect(142, 41, 56, 17, l_text ); //
    tft.cursorToXY(146, 45);
    tft.print(rtc.getTimeStr(FORMAT_SHORT));
    if (old_d != rtc.getDateStr(FORMAT_LONG, FORMAT_LITTLEENDIAN, '/')) {

      tft.fillRect(37, 41, 97, 17, l_text); //l_text
